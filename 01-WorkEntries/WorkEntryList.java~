...
{  
   ...
   */
  public class StatusPath implements Comparable<StatusPath>
  {
  ...
  {    
    // HashSet will generally have constant-time lookup/add/etc
    HashSet<Long> statusList = new HashSet<Long>();
    
    // record all existing statuses in a list (without duplicates)
  ...
  /**
   * Outputs a list, in descending order, of the X users assigned to the most tickets.
   */
  public void printMostActiveUsers(int maxUsersToPrint)
  {
    ...
    // insert entries into sorted list (will sort with high entries earlier in list)
    TreeSet<UserEntries> mostActiveUsers = new TreeSet<UserEntries>();    
    for(Long user : allUsers.keySet())
    {
      mostActiveUsers.add(new UserEntries(user, allUsers.get(user)));
    }
    
    // do the printing
    Iterator<UserEntries> iterator = mostActiveUsers.iterator();
    int i=1;
    while(i <= maxUsersToPrint && iterator.hasNext())
    {
      UserEntries data = iterator.next();
      System.out.println("" + i + ". user" + data.user_id + ": " + data.entries);
      i++;
    }
  }
